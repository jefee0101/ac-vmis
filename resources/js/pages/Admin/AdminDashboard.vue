...
const adminNotifications = ref<Array<{
    id: number
    title: string
    message: string
    type: string
...
watch(
    ...
    (items) => {
        adminNotifications.value = Array.isArray(items)
...
<template>
    <div class="admin-shell min-h-screen bg-slate-50 text-slate-900">
        <div
            class="pointer-events-none fixed inset-0 -z-10 bg-[radial-gradient(circle_at_top_right,_rgba(15, 23, 42,0.10),transparent_45%)]" />

        <div v-if="mobileNavOpen" class="fixed inset-0 z-30 bg-slate-900/40 lg:hidden" @click="closeMobileNav" />

        <aside
            class="fixed left-0 top-0 z-40 h-screen border-r border-slate-200 bg-white/95 backdrop-blur transition-[transform,width] duration-300 ease-out will-change-[transform,width]"
            :class="[
            ...
            <div class="flex h-full flex-col">
                <div class="admin-shell__sidebar-header flex items-start justify-between gap-2 border-b border-slate-200 px-4 py-4">
                    <div class="min-w-0">
                        <p
                            class="admin-shell__kicker text-xs font-semibold uppercase tracking-[0.18em] text-slate-500"
                    ...
                    <button
                        type="button"
                        class="admin-shell__nav-toggle hidden h-9 w-9 items-center justify-center rounded-lg border md:inline-flex"
                        @click="toggleSidebar"
                ...
                <nav class="flex-1 space-y-1 overflow-y-auto p-3">
                    <p v-if="!sidebarCollapsed"
                        class="px-2 pb-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-400">
                        Workspace
                    </p>
                    <button v-for="entry in pages" :key="entry.name" type="button" @click="goTo(entry.route)"
                        ...
                        <svg class="h-4.5 w-4.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                            ...
                            <path v-for="(path, idx) in entry.iconPaths" :key="`${entry.route}-icon-${idx}`"
                                :d="path" />
                        </svg>
                        <span
                ...
                <div class="border-t border-slate-200 px-3 py-3">
                    ...
                    <button type="button"
                        ...
                        :class="sidebarCollapsed && !mobileNavOpen ? 'justify-center' : ''" @click="logout">
                        <svg class="h-4.5 w-4.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                            stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                ...
                <div class="mx-auto flex max-w-[1600px] items-center justify-between gap-3 px-4 py-3 sm:px-6">
                    <div class="flex min-w-0 items-center gap-2">
                        <button type="button"
                            ...
                            @click="mobileNavOpen = true" aria-label="Open admin navigation">
                        ...
                        <div class="min-w-0">
                            <p class="admin-shell__kicker truncate text-xs uppercase tracking-[0.16em] text-slate-500">
                                Administrator</p>
                            <h2 class="admin-shell__title truncate text-base font-semibold text-[#1f2937] sm:text-lg">{{
                                currentPageName }}</h2>
                        </div>
                    </div>
                            ...
                            >
                                ...
                                <div class="max-h-72 overflow-y-auto">
                                    <button
                                        ...
                                        @click="markBellRead(item); goTo('/announcements')"
                                    >
                                        <span
                ...
                <div v-else-if="dashboard" class="space-y-5">
                    ...
                    <section class="grid grid-cols-1 gap-3 md:grid-cols-6">
                        <article class="rounded-full border border-[#034485]/45 bg-white p-4">
                            ...
                            <p class="mt-1 text-2xl font-bold text-emerald-700">{{ dashboard.kpis.attendance_rate }}%
                            </p>
                        </article>
                        <article class="rounded-full border border-amber-200 bg-amber-50 p-4">
                            <p class="text-xs text-amber-700">No Response</p>
                            <p class="mt-1 text-2xl font-bold text-amber-900">{{ dashboard.kpis.no_response }}</p>
                        </article>
                        <article class="rounded-full border border-rose-200 bg-rose-50 p-4">
                        ...
                        <article class="rounded-full border border-orange-200 bg-orange-50 p-4">
                        ...
                        <article class="rounded-full border border-slate-200 bg-slate-50 p-4">
                    ...
                    <section class="grid grid-cols-1 gap-4 xl:grid-cols-3">
                        <article class="rounded-xl border border-[#034485]/45 bg-white p-4 xl:col-span-2">
                            <div class="mb-3 flex items-center justify-between">
                                ...
                                <div class="flex gap-2 text-xs">
                                    <span class="inline-flex items-center gap-1 text-emerald-700"><span
                                            class="h-2 w-2 rounded-full bg-emerald-500" />Present</span>
                                    <span class="inline-flex items-center gap-1 text-amber-700"><span
                                            class="h-2 w-2 rounded-full bg-amber-500" />Late</span>
                                    <span class="inline-flex items-center gap-1 text-red-700"><span
                                            class="h-2 w-2 rounded-full bg-red-500" />Absent</span>
                                    <span class="inline-flex items-center gap-1 text-slate-700"><span
                                            class="h-2 w-2 rounded-full bg-slate-500" />No Response</span>
                                </div>
                            </div>
                            <div class="overflow-x-auto">
                                <svg viewBox="0 0 620 210"
                                    class="h-[220px] min-w-[620px] w-full rounded-lg bg-slate-50">
                                    <polyline :points="buildPolyline(dashboard.trends.attendance.present)" fill="none"
                                        stroke="#10b981" stroke-width="2.5" stroke-linecap="round" />
                                    <polyline :points="buildPolyline(dashboard.trends.attendance.late)" fill="none"
                                        stroke="#f59e0b" stroke-width="2.5" stroke-linecap="round" />
                                    <polyline :points="buildPolyline(dashboard.trends.attendance.absent)" fill="none"
                                        stroke="#ef4444" stroke-width="2.5" stroke-linecap="round" />
                                    <polyline :points="buildPolyline(dashboard.trends.attendance.no_response)"
                            ...
                            <div class="mt-2 grid grid-cols-4 gap-1 text-[10px] text-slate-500 sm:grid-cols-8">
                                <span v-for="label in dashboard.trends.labels" :key="label" class="truncate">{{ label
                                    }}</span>
                            </div>
                        </article>
                        ...
                        <article class="rounded-xl border border-[#034485]/45 bg-white p-4">
                            ...
                            <div class="space-y-2 text-xs">
                                <div>
                                    <div class="mb-1 flex justify-between"><span class="text-slate-600">Fit</span><span
                                            class="font-semibold text-slate-900">{{
                                            dashboard.trends.health_distribution.fit }}</span></div>
                                    <div class="h-2 rounded-full bg-slate-100">
                                        <div class="h-2 rounded-full bg-emerald-500"
                                            :style="{ width: `${healthPercent(dashboard.trends.health_distribution.fit, Object.values(dashboard.trends.health_distribution).reduce((a, b) => a + b, 0))}%` }" />
                                    </div>
                                </div>
                                <div>
                                    <div class="mb-1 flex justify-between"><span class="text-slate-600">Fit w/
                                            Restriction</span><span class="font-semibold text-slate-900">{{
                                                dashboard.trends.health_distribution.fit_with_restrictions }}</span></div>
                                    <div class="h-2 rounded-full bg-slate-100">
                                        <div class="h-2 rounded-full bg-amber-500"
                                            :style="{ width: `${healthPercent(dashboard.trends.health_distribution.fit_with_restrictions, Object.values(dashboard.trends.health_distribution).reduce((a, b) => a + b, 0))}%` }" />
                                    </div>
                                </div>
                                <div>
                                    <div class="mb-1 flex justify-between"><span class="text-slate-600">Not
                                            Fit</span><span class="font-semibold text-slate-900">{{
                                            dashboard.trends.health_distribution.not_fit }}</span></div>
                                    <div class="h-2 rounded-full bg-slate-100">
                                        <div class="h-2 rounded-full bg-orange-500"
                                            :style="{ width: `${healthPercent(dashboard.trends.health_distribution.not_fit, Object.values(dashboard.trends.health_distribution).reduce((a, b) => a + b, 0))}%` }" />
                                    </div>
                                </div>
                                <div>
                                    <div class="mb-1 flex justify-between"><span
                                            class="text-slate-600">Expired</span><span
                                            class="font-semibold text-slate-900">{{
                                            dashboard.trends.health_distribution.expired }}</span></div>
                                    <div class="h-2 rounded-full bg-slate-100">
                                        <div class="h-2 rounded-full bg-rose-500"
                                            :style="{ width: `${healthPercent(dashboard.trends.health_distribution.expired, Object.values(dashboard.trends.health_distribution).reduce((a, b) => a + b, 0))}%` }" />
                                    </div>
                                </div>
                            </div>
                        </article>
                    ...
                    <section class="grid grid-cols-1 gap-4 xl:grid-cols-2">
                        <article class="rounded-xl border border-[#034485]/45 bg-white p-4">
                            ...
                            <div v-if="dashboard.trends.academic_by_team.length === 0" class="text-sm text-slate-500">No
                                evaluations found for current period.</div>
                            <div v-else class="space-y-2">
                                <div v-for="team in dashboard.trends.academic_by_team" :key="team.team_name"
                                    ...
                                    <div class="flex h-2 overflow-hidden rounded-full bg-slate-100">
                                        <span class="bg-emerald-500"
                                            :style="{ width: `${academicSegment(team.eligible, team.total)}%` }" />
                                        <span class="bg-amber-500"
                                            :style="{ width: `${academicSegment(team.probation, team.total)}%` }" />
                                        <span class="bg-red-500"
                                            :style="{ width: `${academicSegment(team.ineligible, team.total)}%` }" />
                                    </div>
                                </div>
                            </div>
                        </article>
                        ...
                        <article class="rounded-xl border border-[#034485]/45 bg-white p-4">
                            <div class="mb-3 flex items-center justify-between">
                                ...
                                <div class="flex gap-2 text-xs">
                                    <span class="inline-flex items-center gap-1 text-emerald-700"><span
                                            class="h-2 w-2 rounded-full bg-emerald-500" />On-time</span>
                                    <span class="inline-flex items-center gap-1 text-amber-700"><span
                                            class="h-2 w-2 rounded-full bg-amber-500" />Late</span>
                                    <span class="inline-flex items-center gap-1 text-slate-700"><span
                                            class="h-2 w-2 rounded-full bg-slate-500" />No Response</span>
                            ...
                            <div v-else class="overflow-x-auto">
                                <div class="min-w-[560px]">
                                    <div class="flex h-44 items-end gap-3">
                                        <div v-for="item in weeklySummary" :key="item.label"
                                            class="flex min-w-[36px] flex-1 flex-col items-stretch">
                                            <div class="flex h-40 flex-col-reverse overflow-hidden rounded-md bg-slate-100"
                                                :title="`On-time ${item.onTime} • Late ${item.late} • No Response ${item.noResponse}`">
                                                <div class="bg-emerald-500"
                                                    :style="{ height: `${weeklyBarHeight(item.onTime)}px` }" />
                                                <div class="bg-amber-500"
                                                    :style="{ height: `${weeklyBarHeight(item.late)}px` }" />
                                                <div class="bg-slate-500"
                                                    :style="{ height: `${weeklyBarHeight(item.noResponse)}px` }" />
                                            </div>
                                            <span class="mt-2 text-center text-[10px] text-slate-500">{{ item.label
                                                }}</span>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <section class="grid grid-cols-1 gap-4 xl:grid-cols-2">
                        <article class="rounded-xl border border-[#034485]/45 bg-white p-4">
                            ...
                            <div v-if="dashboard.queues.today_schedules.length === 0" class="text-sm text-slate-500">No
                                schedules for today.</div>
                            <div v-else class="space-y-2">
                                <div v-for="item in dashboard.queues.today_schedules" :key="item.id"
                                    class="rounded-lg border border-slate-200 p-3">
                                    <div class="flex items-start justify-between gap-2">
                                        <div>
                                            <p class="text-sm font-semibold text-slate-900">{{ item.title }}</p>
                                            <p class="text-xs text-slate-500">{{ item.team_name }} • {{ item.start_time
                                                }}</p>
                                        </div>
                                        <button type="button"
                                            class="rounded-md border border-slate-300 px-2 py-1 text-xs hover:bg-slate-100"
                                            @click="goTo('/operations?tab=calendar')">Open</button>
                                    </div>
                                    <div class="mt-2 grid grid-cols-3 gap-2 text-xs">
                                        <p class="rounded bg-amber-50 px-2 py-1 text-amber-700">Late: {{ item.late }}
                                        </p>
                                        <p class="rounded bg-rose-50 px-2 py-1 text-rose-700">Absent: {{ item.absent }}
                                        </p>
                                        <p class="rounded bg-slate-100 px-2 py-1 text-slate-700">No Response: {{
                                            item.no_response }}</p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <article class="rounded-xl border border-[#034485]/45 bg-white p-4">
                            ...
                            <div v-if="dashboard.queues.needs_attention.length === 0" class="text-sm text-slate-500">No
                                priority items right now.</div>
                            <div v-else class="space-y-2">
                                <div v-for="(item, idx) in dashboard.queues.needs_attention"
                                    :key="`${item.type}-${idx}`"
                    ...
                    <section class="rounded-xl border border-[#034485]/45 bg-white p-4">
                        <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
                            ...
                            <div class="flex gap-2">
                                ...
                                <button v-if="!activityCollapsed" type="button"
                                    class="rounded-md border border-slate-300 px-2.5 py-1 text-xs hover:bg-slate-100"
                        ...
                        <div v-if="!activityCollapsed" :class="activityMaximized ? 'max-h-[520px]' : 'max-h-[260px]'"
                            ...
                            <table class="min-w-full text-sm">
                                ...
                                <tbody>
                                    <tr v-for="item in dashboard.activity_log.items" :key="item.id"
                                        ...
                                        <td class="px-3 py-2 text-xs text-slate-600">{{
                                            formatActivityTime(item.happened_at) }}</td>
                                    </tr>
                                    <tr v-if="dashboard.activity_log.items.length === 0">
                                        <td colspan="3" class="px-3 py-8 text-center text-slate-500">No recent
                                            student/coach activity found.</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
