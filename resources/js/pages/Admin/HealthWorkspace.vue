        ...
        <template v-if="isClearance && clearance">
            <section class="rounded-xl border border-[#034485]/45 bg-white p-4">
                <div class="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-5">
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <select v-model="clearanceForm.per_page" class="rounded-md border border-slate-300 px-3 py-2 text-sm"><option value="15">15</option><option value="25">25</option><option value="50">50</option></select>
                        <button type="button" class="rounded-md border border-slate-300 px-3 py-2 text-sm hover:bg-slate-100" @click="reloadClearance(1)">Apply</button>
                        <button type="button" class="rounded-md border border-slate-300 px-3 py-2 text-sm hover:bg-slate-100" @click="resetClearance">Reset</button>
                    </div>
                </div>
            </section>
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <transition-group name="table-fade" tag="tbody">
                            <tr v-for="row in clearance.records.data" :key="row.id" class="border-t border-slate-200 text-slate-700">
                                ...
                                <td class="px-4 py-3"><div>{{ row.clearance_date || '-' }}</div><div class="text-xs text-slate-500">Until: {{ row.valid_until || '-' }}</div></td>
            ...
            <div class="flex items-center justify-between text-sm text-slate-600">
                ...
                <div class="flex gap-2">
                    <button type="button" class="rounded-md border border-slate-300 bg-white px-3 py-1 hover:bg-slate-100 disabled:opacity-40" :disabled="clearance.records.meta.current_page <= 1" @click="reloadClearance(clearance.records.meta.current_page - 1)">Previous</button>
                    <button type="button" class="rounded-md border border-slate-300 bg-white px-3 py-1 hover:bg-slate-100 disabled:opacity-40" :disabled="clearance.records.meta.current_page >= clearance.records.meta.last_page" @click="reloadClearance(clearance.records.meta.current_page + 1)">Next</button>
                </div>
            </div>
        </template>
        ...
        <template v-if="!isClearance && wellness">
            <section class="rounded-xl border border-[#034485]/45 bg-white p-4">
                <div class="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-4">
                    ...
                    <select v-model="wellnessForm.fatigue_min" class="rounded-md border border-slate-300 px-3 py-2 text-sm"><option value="">Fatigue: Any</option><option value="3">Fatigue: 3+</option><option value="4">Fatigue: 4+</option><option value="5">Fatigue: 5</option></select>
                    <label class="inline-flex items-center gap-2 rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-700"><input v-model="wellnessForm.injury_only" type="checkbox" /> Injury Only</label>
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <select v-model="wellnessForm.per_page" class="rounded-md border border-slate-300 px-3 py-2 text-sm"><option value="15">15</option><option value="25">25</option><option value="50">50</option></select>
                        <button type="button" class="rounded-md border border-slate-300 px-3 py-2 text-sm hover:bg-slate-100" @click="reloadWellness(1)">Apply</button>
                        <button type="button" class="rounded-md border border-slate-300 px-3 py-2 text-sm hover:bg-slate-100" @click="resetWellness">Reset</button>
                    </div>
                </div>
            </section>
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <transition-group name="table-fade" tag="tbody">
                            <tr v-for="row in wellness.logs.data" :key="row.id" class="border-t border-slate-200 text-slate-700">
                                ...
                                <td class="px-4 py-3"><div class="font-medium text-slate-900">{{ row.student_name }}</div><div class="text-xs text-slate-500">{{ row.student_id_number || '-' }}</div></td>
                                <td class="px-4 py-3"><div>{{ row.team_name || '-' }}</div><div class="text-xs text-slate-500">{{ row.schedule_title || '-' }} ({{ row.schedule_type || '-' }})</div></td>
            ...
            <div class="flex items-center justify-between text-sm text-slate-600">
                ...
                <div class="flex gap-2">
                    <button type="button" class="rounded-md border border-slate-300 bg-white px-3 py-1 hover:bg-slate-100 disabled:opacity-40" :disabled="wellness.logs.meta.current_page <= 1" @click="reloadWellness(wellness.logs.meta.current_page - 1)">Previous</button>
                    <button type="button" class="rounded-md border border-slate-300 bg-white px-3 py-1 hover:bg-slate-100 disabled:opacity-40" :disabled="wellness.logs.meta.current_page >= wellness.logs.meta.last_page" @click="reloadWellness(wellness.logs.meta.current_page + 1)">Next</button>
                </div>
            </div>
