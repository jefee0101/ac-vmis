...
const sortedSchedules = computed(() =>
    [...(props.schedules || [])].sort((a, b) => +new Date(a.start) - +new Date(b.start))
)
...
const selectedSchedule = computed(() =>
    sortedSchedules.value.find(item => item.id === selectedScheduleId.value) || null
)

const upcomingSchedules = computed(() =>
    sortedSchedules.value.filter((item) => !isPastSchedule(item))
)
...
const needsResponseCount = computed(() =>
    upcomingSchedules.value.filter((item) => !item.attendance_status).length
)

watch(sortedSchedules, (items) => {
...
function setAttendance(scheduleId: number, status: 'present' | 'absent' | 'excused', notes: string | null = null) {
    ...
    router.put(`/Student/Schedules/${scheduleId}/attendance`, { status, notes }, {
        preserveScroll: true,
        onFinish: () => {
...
function changeTeam() {
    ...
    router.get('/MySchedule', { team_id: selectedTeamId.value }, {
        preserveScroll: true,
        preserveState: true,
        replace: true,
    })
        ...
        <div v-else class="space-y-4">
            ...
            <div class="grid grid-cols-1 xl:grid-cols-5 gap-4">
                <section v-if="showCalendar" class="xl:col-span-3 bg-white border border-[#034485]/35 rounded-xl p-4">
                    ...
                    <VueCal sm style="height: 650px" :events="calendarEvents" v-model:view-date="calendarViewDate"
                        v-model:selected-date="calendarSelectedDate" default-view="week" :time="true"
                        :twelve-hour="true" time-format="h:mm {am}" events-on-month-view @event-click="onCalendarEventClick" />
                </section>
                ...
                >
                    <div class="flex items-center justify-between mb-3">
                        <div>
                            <h2 class="text-slate-900 font-semibold">Upcoming Sessions</h2>
                    ...
                    <div v-if="upcomingSchedules.length === 0" class="text-sm text-slate-500">
                        ...
                        <div v-else class="space-y-3">
                            ...
                            <div class="flex items-start justify-between gap-2">
                                ...
                                <div class="flex items-center gap-2">
                                    <span class="text-[10px] px-2 py-0.5 rounded" :class="statusClass(item.attendance_status)">
                                    ...
                                    <button @click="openQrModal(item.id)"
                                        ...
                                        class="px-2.5 py-1 text-xs rounded bg-[#1f2937] hover:bg-[#334155] text-white">
                            ...
                            <div class="mt-2 flex flex-wrap gap-2 text-[11px]">
                                <span class="rounded-full px-2 py-0.5 border" :class="timingClass(item)">{{ timingLabel(item) }}</span>
                                <span v-if="isToday(item)" class="rounded-full px-2 py-0.5 border border-amber-200 bg-amber-50 text-amber-700">Today</span>
                                <span v-if="needsResponse(item)" class="rounded-full px-2 py-0.5 border border-rose-200 bg-rose-50 text-rose-700">Needs response</span>
                            </div>

                            <div class="text-xs text-slate-600 mt-2">{{ formatPHT(item.start) }}</div>
                            ...
                            <div v-if="item.attendance_notes && (item.attendance_status === 'absent' || item.attendance_status === 'excused')"
                            ...
                            <div class="mt-3 pt-3 border-t border-[#034485]/20">
                                ...
                                <div class="flex gap-2">
                                    <button @click="setAttendance(item.id, 'present')"
                                        ...
                                        class="px-3 py-1.5 text-xs rounded bg-emerald-600 hover:bg-emerald-700 text-white disabled:opacity-50">
                                        Attending
                                    </button>
                                    <button @click="openReasonModal(item, 'absent')"
                                        :disabled="processingScheduleId === item.id"
                                        class="px-3 py-1.5 text-xs rounded bg-rose-600 hover:bg-rose-700 text-white disabled:opacity-50">
                                        Not Attending
                                    </button>
                                    <button @click="openReasonModal(item, 'excused')"
                                        :disabled="processingScheduleId === item.id"
                                        class="px-3 py-1.5 text-xs rounded bg-amber-500 hover:bg-amber-600 text-slate-900 disabled:opacity-50">
                                        Excused
                                    </button>
                                </div>
                            </div>
                    ...
                    <div v-if="showCompleted" class="mt-6">
                        ...
                        <div v-if="completedSchedules.length === 0" class="text-sm text-slate-500">
            ...
            <div class="bg-white border border-[#034485]/35 rounded-xl w-full max-w-sm">
                ...
                <div class="p-4 space-y-3">
                    <p class="text-xs text-slate-500">
                        Show this to your coach. QR rotates automatically for security.
                    </p>
                    <p v-if="qrSecondsLeft > 0" class="text-xs text-amber-600">
                        Token refresh in about {{ qrSecondsLeft }}s
                    </p>
                    <p v-if="qrError" class="text-sm text-rose-600">
                        {{ qrError }}
                    </p>
                    <div v-if="qrLoading" class="bg-slate-100 rounded-lg p-3 space-y-2">
                        <Skeleton class="h-5 w-1/2 bg-slate-200" />
                        <Skeleton class="h-60 w-full bg-slate-200" />
                    </div>
                    <div v-if="qrImageUrl" class="bg-white rounded-lg p-3 flex justify-center">
                        <img :src="qrImageUrl" alt="Attendance QR" class="w-60 h-60 object-contain" />
                    </div>
                    <div v-if="qrToken" class="text-[11px] text-slate-400 break-all">
                        Token: {{ qrToken }}
                    </div>
                </div>
            </div>
